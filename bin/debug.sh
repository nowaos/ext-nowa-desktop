...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      echo "  debug.sh -m \"Auto Dark Wallpaper\"    # Filter specific module"
      exit 0
      ;;
    *)
...
# Build grep pattern
if [ -n "$MODULE_FILTER" ]; then
  echo "Looking for 'Nowa Desktop [$MODULE_FILTER]' messages..."
  echo ""
  journalctl -f -o cat /usr/bin/gnome-shell 2>/dev/null | grep --line-buffered "Nowa Desktop \[$MODULE_FILTER\]"
else
  echo "Looking for 'Nowa Desktop' messages..."
  echo ""
  journalctl -f -o cat /usr/bin/gnome-shell 2>/dev/null | grep --line-buffered "Nowa Desktop"
fi
